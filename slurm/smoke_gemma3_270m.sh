#!/bin/bash
#SBATCH --job-name=smoke_gemma3_270m
#SBATCH --partition=gpu
#SBATCH --time=00:30:00
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --gres=gpu:lovelace_l40:2
#SBATCH --output=logs/smoke_gemma3_270m_%j.out
#SBATCH --error=logs/smoke_gemma3_270m_%j.err
...
python -m src.jobs.run_experiment --base-config configs/base.yaml --config configs/smoke_gemma3_270m.yaml --run-id smoke_gemma3_270m_$(date +%Y%m%d_%H%M%S)
